...
assert.equal(gateLegalCommand(chronology, "requires_control", 49, 0).allowed, false, "chronology is blocked without sources");
assert.equal(gateLegalCommand(chronology, "requires_control", 49, 1).allowed, true, "chronology opens with sources below full coverage");
assert.equal(gateLegalCommand(evidence, "requires_control", 50, 1).allowed, true, "evidence opens with sources");
assert.equal(gateLegalCommand(risk, "requires_control", 79, 0).allowed, false, "risk waits for sources");
assert.equal(gateLegalCommand(risk, "requires_control", 1, 1).allowed, true, "risk opens preliminarily with sources");
assert.equal(gateLegalCommand(quality, "not_ready", 94, 1).allowed, false, "quality waits for a non-not-ready verdict");
assert.equal(gateLegalCommand(quality, "ready_for_draft_control", 95, 1).allowed, true, "quality opens with sources and non-blocked readiness");
