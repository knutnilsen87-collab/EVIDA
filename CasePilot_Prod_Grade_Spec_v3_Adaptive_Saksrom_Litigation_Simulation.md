...
## A.21 Rettssakssimulering / Litigation Simulation Mode

### Purpose

Evida must support **rettssakssimulering** as a preparation and training tool inside Saksrom.

This mode is intended to help legal users test whether a case, argument, witness strategy, evidence structure or procedural position holds together before real proceedings.

Evida must never present litigation simulation as a replacement for an attorney's legal, procedural or ethical assessment.

### Core Positioning

Use this product framing:

> Rettssakssimulering hjelper deg å teste saken fra flere perspektiver: retten, motparten, vitner, prosedyre, forlik og kvalitetskontroll. Simuleringen bygger på dokumenterte kilder der de finnes, markerer usikkerhet og viser hva som må kontrolleres manuelt før faktisk bruk.

### Supported Simulation Types

Evida must support the following simulation modes:

1. `trial_hearing` — simulated hovedforhandling
2. `judge_panel` — simulated judge panel / dommerspørsmål
3. `opposing_counsel` — simulated motpartens advokat
4. `cross_examination` — simulated kryssforhør
5. `direct_examination` — simulated direkte eksaminasjon
6. `closing_argument_test` — prosedyretest
7. `judgment_simulation` — simulated dom
8. `settlement_simulation` — forlikssimulering
9. `roleplay` — role-based courtroom simulation
10. `final_litigation_quality_check` — strict quality review before procedural use

### Required Commands

Add these commands to the Saksrom command system:

```text
'prosess          — structured litigation preparation overview
'hovedforhandling — simulated main hearing
'dommer           — simulated judge questions
'motpart          — simulated opposing counsel attack
'kryssforhor      — cross-examination plan
'direkte          — direct examination plan
'prosedyre        — procedure / closing argument test
'dom              — simulated judgment
'forlik           — settlement simulation
'rolle            — roleplay mode
```

Existing related commands must remain available:
...
Commands are optional. Users must also be able to trigger the same modes through natural language:

```text
Simuler dommeren og still meg 10 kritiske spørsmål.
Vær motpartens advokat og angrip anførslene mine.
Lag kryssforhør for motparten.
Test prosedyren min.
Hvordan kan retten vurdere denne saken?
Simuler forliksforhandling.
Hva ville en kritisk dommerpanel spurt om?
```

### Suggested Action Access

After an answer in Saksrom, Evida may show suggested actions:

```text
Mulige neste steg:
1. Simuler dommerens spørsmål
2. Angrip saken som motpart
3. Lag kryssforhør
4. Test prosedyren
```

The user must be able to reply with:

```text
1
ta 2
se på punkt 3
gå videre med den fjerde
```

and Evida must resolve the response to the correct suggested simulation action.

### Standard Simulation Setup

When the user asks for litigation simulation without enough detail, Evida should ask for or infer the following setup:

```text
RETTSAKSSIMULERING

Sakstype:
Rolle jeg skal simulere:
Prosessform:
Sentrale anførsler:
Dokumentgrunnlag:
Ønsket intensitet:
...
Output:
1. Kritiske spørsmål
2. Forventede motargumenter
3. Bevisrisiko
4. Rettslig risiko
5. Anbefalt justering
6. Ny simulert runde
```

If sufficient case context already exists, Evida should proceed and explicitly state assumptions instead of blocking.

### Intensity Levels

Simulation must support intensity settings:

```text
mild          — supportive review, identifies obvious gaps
realistisk    — balanced court/opposing-side simulation
aggressiv     — strong adversarial attack
dommerkritisk — focuses on what a judge/panel is likely to question
```

Default intensity: `realistisk`.

### Role Modes

Evida must support the following roles:

```text
1. Dommer
2. Motpartens advokat
3. Egen advokat
4. Klient
5. Vitne
6. Sakkyndig
7. Forliksmekler
8. Lagdommerpanel
9. Kritisk kvalitetskontrollør
```

Each role must have a defined response behavior.
...
1. Hvilket konkret rettslig grunnlag bygger kravet på?
...
5. Hvorfor er dette ikke bare bevistvil, men et rettslig holdbart krav?
6. Hvordan håndterer De motpartens dokumentasjon?
7. Hva er Deres beste rettskilde for dette punktet?
```

#### Motpartens advokat

Focus:
- alternative fact explanations
- evidentiary objections
- procedural objections
- limitation/deadline issues
- causation
- loss quantification
- unclear claims
- weak documentation
- adverse legal sources
...
1. Det ikke foreligger dokumentert mislighold.
2. Reklamasjon kom for sent.
3. Tapet er ikke sannsynliggjort.
4. Årsakssammenheng er ikke dokumentert.
5. Saksøkers egen opptreden har medvirket til tapet.

Prosessuell bruk:
Dette bør møtes allerede i innledningsforedraget og ikke først i replikk.
```
...
- evidentiary leverage
- negotiation risk
- face-saving settlement structures
...
Prosesskostnader og usikker tapsutmåling.

Vår pressfaktor:
Sterk dokumentasjon på ansvar.

Anbefalt strategi:
Fremsett et begrunnet tilbud med tydelig dokumentert minimumskrav og kostnadsrisiko.
```

### Simulated Main Hearing Structure

For `'hovedforhandling`, Evida should produce:
...
7. Sakkyndige
8. Prosedyre
9. Replikk og duplikk
10. Dommerspørsmål
11. Foreløpig risikovurdering
```
...
For `'kryssforhor`, Evida must support cross-examination for:
...
Formål:
[Hva spørsmålene skal etablere.]

Kildegrunnlag:
[Dokumenter/sider/fakta som underbygger spørsmålet.]

Risiko:
[Hva spørsmålet kan åpne for.]
...
Spørsmål:
1. De mottok e-posten 14. mai, korrekt?
2. De svarte ikke skriftlig før 28. mai?
3. I mellomtiden fortsatte De leveransen?
4. Det finnes ingen dokumentasjon på at De protesterte tidligere?

Formål:
Å etablere passivitet og kunnskap.

Risiko:
Spørsmål 2 kan åpne for forklaring om telefonsamtale. Bør følges opp med krav om dokumentasjon.
```

### Direct Examination Mode

For `'direkte`, Evida must help build non-leading questions for own client or witnesses.

Standard output:

```text
DIREKTE EKSAMINASJON – [VITNE/KLIENT]
...
Dokumentkobling:
[Hvor vitnet bør knyttes til dokumentbevis.]

Risiko:
[Hva som kan være uklart, overdrevet eller sårbart.]
```

Example:

```text
DIREKTE EKSAMINASJON – KLIENT

Mål:
Få frem faktum kronologisk, nøkternt og dokumentnært.

Spørsmål:
1. Når ble avtalen inngått?
2. Hva ble avtalt om levering?
3. Hva skjedde på leveringsdagen?
4. Når oppdaget du avviket?
5. Hvordan varslet du motparten?
6. Hvilke konsekvenser fikk avviket?
```

### Procedure Test Mode

For `'prosedyre`, Evida must test:
- critical judge questions
- likely opposing reply
- weak points in the structure
- what the judge is likely to care about
- which arguments should be prioritized
- which arguments should be toned down
...
### Judgment Simulation Mode

For `'dom`, Evida may simulate how a judgment might look based on the available material.
...
For `'forlik`, Evida must help structure settlement strategy without presenting financial/legal outcome guarantees.
...
Beste realistiske utfall:
[område, hvis bruker har gitt krav/verdi]

Sannsynlig forlikssone:
[område, med tydelig usikkerhet]
...
[hva må undersøkes før tilbud]
```

### Readiness Gating

Litigation simulation must be gated by case readiness.

#### No usable sources

Allowed:
- explain litigation simulation
- ask for facts
- ask user to import documents
- simulate only as hypothetical / user-provided facts

Blocked:
- source-grounded litigation assessment
- simulated judgment based on documents
- final quality or procedural strategy

#### Has sources

Allowed:
- preliminary judge questions
- preliminary opposing counsel attack
- cross-examination planning
- direct examination planning
- chronology-based litigation preparation
...
- claim/argument attack
- evidence-to-issue simulation

#### Draft ready

Allowed:
- simulated main hearing
- judgment simulation
- final litigation quality check
- strict procedural readiness review

If the requested mode is blocked, Evida must explain why and offer the safest next action.

Example:

```text
Jeg kan ikke simulere en full dom ennå, fordi kildegrunnlaget krever kontroll.
Jeg kan først hjelpe deg med:
1. dommerspørsmål basert på foreløpige kilder
2. motpartens mulige angrep
3. bevisrisiko
4. hva som må styrkes før domssimulering
```

### Source and Risk Rules

Every litigation simulation output must include:

```text
KILDESTATUS
- Hvilke deler bygger på dokumenterte kilder
- Hvilke deler bygger på brukeropplysninger
- Hvilke deler er hypotetiske
- Hvilke punkter mangler dokumentasjon
...
If any part relies on unverified facts, Evida must mark it:

```text
RISIKOVARSEL:
Denne delen bygger på udokumentert faktum. Må støttes av dokument, vitneforklaring eller annen bevisførsel før bruk i prosess.
```

### Legal Source Warning

When simulation touches legal sources, precedents or current law, Evida must include:
...
- imply that lawyer review is optional
- give procedural certainty when dates/deadlines are not verified
- cross-case learn sensitive litigation strategy without consent

Evida must:
- mark simulations as preparation/training
- separate documented facts from hypotheticals
- identify weak points and missing sources
- recommend manual attorney review
- preserve sources, uncertainty and next steps
- keep all simulation context case-local by default

### Recommended Litigation Preparation Workflow

Evida should recommend this flow:

```text
1. Last inn dokumenter
2. Lag kronologi
3. Lag bevisliste
4. Lag anførselstavle
5. Kjør motargumenter
6. Kjør kryssforhør
7. Kjør dommersimulering
8. Kjør kvalitet/endelig før bruk
```
...
All litigation simulation outputs should end with:
...
type LitigationSimulationRequest = {
  mode: LitigationSimulationMode;
  role?: LitigationRole;
  intensity: SimulationIntensity;
  caseType?: string;
  proceduralForm?: string;
  centralClaims?: string[];
  documentScope?: string[];
  requestedOutput?: string[];
  sourceTurnId?: string;
  selectedSuggestedActionId?: string;
};

type LitigationSimulationOutput = {
  mode: LitigationSimulationMode;
  role?: LitigationRole;
  intensity: SimulationIntensity;
  summary: string;
  ...
  nextSuggestedActions: SuggestedAction[];
};
```

### Acceptance Criteria

- User can trigger litigation simulation through natural language, buttons, numeric follow-up or commands.
- `'dommer` produces judge-style critical questions.
- `'motpart` produces opposing counsel attack.
- `'kryssforhor` produces cross-examination plan with purpose, source basis and risk.
- `'direkte` produces direct examination plan.
- `'prosedyre` tests procedure/closing argument.
- `'dom` produces clearly marked simulated judgment, not prediction.
- `'forlik` produces settlement simulation with uncertainty and no guarantees.
- Simulation intensity changes output behavior.
- All outputs separate documented facts from hypothetical/user-provided facts.
- All outputs include source status, risk warning and next steps.
- Modes are gated by readiness.
- Final/procedure-heavy simulations are blocked or limited when source coverage is insufficient.
- Legal source warnings are shown when legal sources/current law are involved.
- Simulation context remains case-local by default.
